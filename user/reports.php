...
// 2. Detect if date column exists
$dateColumn = null;
$checkColumnQuery = "SHOW COLUMNS FROM products LIKE 'date_added'";
$checkResult = $conn->query($checkColumnQuery);
if ($checkResult->num_rows > 0) {
    $dateColumn = 'date_added';
} else {
    $checkColumnQuery = "SHOW COLUMNS FROM products LIKE 'created_at'";
    $checkResult = $conn->query($checkColumnQuery);
    if ($checkResult->num_rows > 0) {
...
// 3. Fetch monthly inventory value (pretty month names)
if ($dateColumn) {
    $inventoryGrowthQuery = "
        SELECT 
            DATE_FORMAT($dateColumn, '%M') AS month_label,
...
$inventoryGrowthResult = $conn->query($inventoryGrowthQuery);


// 4. Prepare data arrays (fill missing months with 0)
$monthlyValues = array_fill(1, 12, 0);

while ($row = $inventoryGrowthResult->fetch_assoc()) {
    $monthNum = (int)$row['month_num'];
    $monthlyValues[$monthNum] = (float)$row['total_value'];
}
...
<div class="main-content inventory-page">
    ...
    <!-- Monthly Inventory Growth Chart -->
    <div class="chart-card">
        <div class="chart-title">🥈 Monthly Inventory Growth</div>
        <div class="chart-subtitle">Visualize your inventory value trends throughout the year.</div>
        <canvas id="inventoryGrowthChart" style="width:100%; height:500px;"></canvas>
    </div>
</div>
...
<script>
const ctx = document.getElementById('inventoryGrowthChart').getContext('2d');
...
const inventoryGrowthChart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: <?php echo json_encode($months); ?>,
        datasets: [{
            label: 'Total Inventory Value (₱)',
            data: <?php echo json_encode($totalValues, JSON_NUMERIC_CHECK); ?>,
            borderColor: '#3b82f6',
    ...
    options: {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Inventory Value Trends - ' + new Date().getFullYear(),
                color: '#0f172a',
                font: {
                    size: 20,
                    weight: 'bold'
